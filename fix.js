...
// Let's remove ALL characters between the FIRST occurrence of "const performFullSync" and "const checkPreRomaneiosCloud = async () => {"
// First, find the very first index
const firstIdx = content.indexOf('const performFullSync = async');

// Second, find the function after the ones we want to replace
const nextFuncIdx = content.indexOf('const checkPreRomaneiosCloud = async () => {');

// The prefix is everything before the first occurrence (back to the beginning of the line)
const lineStartIdx = content.lastIndexOf('\n', firstIdx);

const newContent = content.substring(0, lineStartIdx + 1) + newPerformSync + "\n\n  " + content.substring(nextFuncIdx);
