...
export async function partitionCtrcs(
  localCtrcs: Ctrc[],
): Promise<{ available: Ctrc[]; linked: Ctrc[]; allImported: Ctrc[] }> {
  ...
  for (const ctrc of localCtrcs) {
    ...
    if (
      activeImportBatchId &&
      ctrc.importBatchId &&
      ctrc.importBatchId !== activeImportBatchId
    ) {
      continue;
    }
    ...
    const hasPreRomaneioId =
      preRomaneioId && activePreRomaneioIds.has(preRomaneioId);
    const hasRomaneioId = !!(ctrc as any).romaneioId;
    const isSelectedPermanently = !!(ctrc as any).selectedForRoute;

    const hasLinkedStatus =
      ctrc.status && notAvailableStatuses.has(ctrc.status);
...
export default function App() {
  ...
  const [pendingTargetView, setPendingTargetView] = useState<ViewType | null>(
    null,
  );
  ...
  const [drivers, setDrivers] = useState<DriverScore[]>(
    IS_DEMO_MODE ? initialDrivers : [],
  );
  const [availableCtrcs, setAvailableCtrcs] = useState<Ctrc[]>(
    IS_DEMO_MODE ? initialAvailableCtrcs : [],
  );
  const [linkedCtrcs, setLinkedCtrcs] = useState<Ctrc[]>(
    IS_DEMO_MODE ? initialLinkedCtrcs : [],
  );
    ...
    return [
      {
        ...
        observations:
          "Roteirizado sob encomenda prioritária. Checar cubagem traseira.",
      },
    ];
  });
  const [expenses, setExpenses] = useState<Expense[]>(
    IS_DEMO_MODE ? initialExpenses : [],
  );
  const [tickets, setTickets] = useState<Ticket[]>(
    IS_DEMO_MODE ? initialTickets : [],
  );
  const [clients, setClients] = useState<CriticClient[]>(
    IS_DEMO_MODE ? initialCriticalClients : [],
  );
  ...
  const [activeRomaneioVehicleId, setActiveRomaneioVehicleId] = useState<
    string | null
  >("RTA3G45");
    ...
    async function initLocalDB() {
      try {
        ...
        if (oldVersion !== APP_VERSION) {
          console.log(
            `[Version Bump] Migrando de ${oldVersion || "vazio"} para ${APP_VERSION}.`,
          );
        ...
        setTimeout(async () => {
          try {
            const { vehicleSupabaseRepository } =
              await import("./infrastructure/supabase/repositories/vehicleSupabaseRepository");
            const { vehicleGrRuleSupabaseRepository } =
              await import("./infrastructure/supabase/repositories/vehicleGrRuleSupabaseRepository");
            const { criticalClientSupabaseRepository } =
              await import("./infrastructure/supabase/repositories/criticalClientSupabaseRepository");

            // Sync GR Rules
            const remoteRulesRes =
              await vehicleGrRuleSupabaseRepository.getAllRules();
            if (
              remoteRulesRes.success &&
              remoteRulesRes.data &&
              remoteRulesRes.data.length > 0
            ) {
              const remoteRules = remoteRulesRes.data.map((r) => ({
                ...
                requiresTrackingAboveValue: Boolean(
                  r.requires_tracking_above_value,
                ),
                requiresAuthorizationAboveLimit:
                  r.requires_authorization_above_limit,
            ...
            // Sync Vehicles
            const remoteVehiclesRes =
              await vehicleSupabaseRepository.getAllVehicles();
            if (
              remoteVehiclesRes.success &&
              remoteVehiclesRes.data &&
              remoteVehiclesRes.data.length > 0
            ) {
            ...
            const remoteClientsRes =
              await criticalClientSupabaseRepository.getAllClients();
            if (
              remoteClientsRes.success &&
              remoteClientsRes.data &&
              remoteClientsRes.data.length > 0
            ) {
            ...
            console.warn(
              "[Sync] Supabase sync initialization failed (offline mode):",
              e,
            );
          }
          ...
          try {
            ...
            console.warn(
              "[Sync] Failed to process background sync queue:",
              err,
            );
          }
        }, 0);
        ...
        if (localRomaneios.length > 0) {
          const sorted = [...localRomaneios].sort((a, b) =>
        ...
        if (localCtrcs.length > 0) {
          const { available, linked, allImported } =
            await partitionCtrcs(localCtrcs);
          ...
          if (ENABLE_ROUTER_DIAGNOSTICS) {
            ...
            localCtrcs.forEach((c) => {
              statusCounts[c.status || "sem status"] =
                (statusCounts[c.status || "sem status"] || 0) + 1;
        ...
        console.error(
          "[App] Falha crítica de inicialização IndexedDB, usando memória:",
          err,
        );
      }
    }
  ...
  const handleUpdateVehicleRegistry = async (vr: VehicleRegistry) => {
    ...
    if (oldVr) {
      ...
      if (oldVr.statusOperacional !== vr.statusOperacional) {
        ...
        if (oldVr.tipo !== vr.tipo)
          changes.push(`tipo (${oldVr.tipo} -> ${vr.tipo})`);
        if (oldVr.limiteGrSugerido !== vr.limiteGrSugerido)
          changes.push(
            `limite GR (${oldVr.limiteGrSugerido} -> ${vr.limiteGrSugerido})`,
          );
        if (oldVr.motoristaPadrao !== vr.motoristaPadrao)
          changes.push(
            `motorista (${oldVr.motoristaPadrao || "Nenhum"} -> ${vr.motoristaPadrao || "Nenhum"})`,
          );
  ...
  const handleUpdateGrRule = async (rule: VehicleGrRule) => {
    ...
    if (oldRule) {
      ...
      if (
        oldRule.requiresTrackingAboveValue !== rule.requiresTrackingAboveValue
      ) {
        await AuditLogRepository.log({
          user: adminProfile.name,
          isMaster: isSessionUnlocked,
      ...
      if (
        oldRule.requiresAuthorizationAboveLimit !==
        rule.requiresAuthorizationAboveLimit
      ) {
        await AuditLogRepository.log({
          user: adminProfile.name,
          isMaster: isSessionUnlocked,
  ...
  const handleUpdateOccurrence = async (o: DeliveryOccurrence) => {
    setOccurrences((prev) =>
      prev.map((item) => (item.codigo === o.codigo ? o : item)),
    );
    await OccurrenceRepository.put(o);
    setIsSyncing(true);
    await syncOccurrenceToSupabase(o);
  ...
  const handleBulkImportOccurrences = async (
    list: DeliveryOccurrence[],
    replaceMode?: boolean,
  ) => {
    if (replaceMode) {
      setOccurrences(list);
      ...
      setOccurrences((prev) => {
        const filteredPrev = prev.filter(
          (p) => !list.some((l) => l.codigo === p.codigo),
        );
  ...
  const handleUpdateCurvaA = async (c: CurvaAClient) => {
    setCurvaAClients((prev) =>
      prev.map((item) => (item.cnpj_remetente === c.cnpj_remetente ? c : item)),
    );
    setIsSyncing(true);
    await syncCurvaAClientToSupabase(c);
    setIsSyncing(false);
  ...
  const handleRemoveCurvaA = async (cnpj: string) => {
    setCurvaAClients((prev) =>
      prev.filter((item) => item.cnpj_remetente !== cnpj),
    );
    setIsSyncing(true);
  ...
  const handleBulkImportCurvaA = async (list: CurvaAClient[]) => {
    setCurvaAClients((prev) => {
      const filteredPrev = prev.filter(
        (p) => !list.some((l) => l.cnpj_remetente === p.cnpj_remetente),
      );
      return [...filteredPrev, ...list];
    });
    setIsSyncing(true);
  ...
  const handleAddCtrcs = async (newCtrcs: Ctrc[]) => {
    ...
    const { available: oldAvailable, linked: oldLinked } =
      await partitionCtrcs(allExistingCtrcs);
    const oldAvailableCount = oldAvailable.length;

    // Apply importBatchId and planningDate to newCtrcs
    const userUnid = adminProfile?.unid || DEFAULT_OPERATIONAL_UNIT;
    ...
    const existingMap = new Map<string, Ctrc>(
      existingCtrcs.map((c) => [c.id, c]),
    );
    ...
    localStorage.setItem(
      ...
      String(oldAvailableCount),
    );
    localStorage.setItem(
      "last_import_new_added_count",
      String(newCtrcs.length),
    );
    ...
    try {
      ...
      for (const ctrc of mergedCtrcs) {
        ...
        const history = await CtrcOccurrenceHistoryRepository.getByCtrcId(
          ctrc.id,
        );
      ...
      if (historyItemsToSave.length > 0) {
        await CtrcOccurrenceHistoryRepository.upsertManyDeduped(
          historyItemsToSave,
        );
        console.log(
          `[Importacao] Gravados ${historyItemsToSave.length} novos eventos no histórico.`,
        );
      }
    } catch (err) {
      console.error(
        "[Importacao] Erro durante o salvamento do histórico de ocorrências do CTRC:",
        err,
      );
    }
    ...
    try {
      ...
      if (allLocalCtres.length > 0) {
        const { available, linked, allImported } =
          await partitionCtrcs(allLocalCtres);
      ...
      console.log(
        `[Importacao] Memória reidratada explicitamente com ${allLocalCtres.length} CTRCs do banco.`,
      );

      // CR-MVP-SUPABASE-06: Sincronização com Supabase (Em Background)
      setTimeout(async () => {
        try {
          const { importBatchSupabaseRepository } =
          ...
          const shipmentsToSync = mergedCtrcs.map((c) => ({
            ...
            delivery_date: parseToIso(c.realDeliveryDate) || null,
            ...
            is_delivered: !!c.realDeliveryDate,
            is_curve_a: c.type === "CURVA A",
            is_critical_client: false,
            is_subcontract: c.isSubcontract,
            origin_series: c.originSeries,
            counts_for_performance: c.countsForDeliveryPerformance,
          ...
          console.error(
            "[Importacao] Falha na sincronização Supabase (fallback ativo):",
            syncErr,
          );
        }
      }, 0);
      ...
      console.error(
        "[Importacao] Falha na recarga explícita pós-importação:",
        rehydrateErr,
      );
    }
  };
  ...
  const handleConsolidateRomaneio = async (
    vehicleId: string,
    assignedCtrcs: Ctrc[],
  ) => {
  ...
  const handleGeneratePreRomaneioSuccess = async (
    ...
    const updatedCtrcs = originalCtrcs.map((c) => ({
      ...c,
    ...
    setAvailableCtrcs((prev) =>
      prev.filter((c) => !preSeparatingIds.includes(c.id)),
    );
  ...
  // ---------------------------------------------------------
  const handleUpdateCtrcStatus = async (
    id: string,
        ...
        if (c.id === clientId) {
          ...
          setTimeout(async () => {
            try {
              const { criticalClientSupabaseRepository } =
                await import("./infrastructure/supabase/repositories/criticalClientSupabaseRepository");
              await criticalClientSupabaseRepository.upsertClient(
                updatedClient,
              );
              await SyncQueueRepository.processSyncQueue();
            } catch (err) {
  ...
  const handleNavigateFromDash = (
    view: "importacao" | "frota" | "roteirizacao",
  ) => {
    setCurrentView(view);
  };
  ...
  const renderActiveView = () => {
        ...
        <div className="flex items-center justify-center min-h-[calc(100vh-12rem)] px-4 py-8">
          <div className="bg-surface-container border border-outline-variant max-w-md w-full rounded-2xl p-6 md:p-8 text-center shadow-2xl relative overflow-hidden">
            ...
            <p className="text-xs text-[var(--router-text-soft)] mb-6 leading-relaxed">
              Você está tentando acessar uma tela de configuração ou auditoria
              restrita. Insira a senha mestre para desbloquear o acesso.
            </p>
    ...
    switch (currentView) {
      ...
      case "finalizacao":
        return (
          <FinalizacaoView
            ...
            activeVehicle={vehicles.find(
              (v) => v.id === activeRomaneioVehicleId,
            )}
      ...
      case "cidades_rotas":
        return <CidadesRotasView isMaster={isSessionUnlocked} />;
      case "regras_gr":
        return (
          <RegrasGrView
            adminUser={
              adminProfile
                ? { ...adminProfile, is_master: isSessionUnlocked }
                : null
            }
            rules={grRules}
      ...
      case "base_dados":
        return (
          <BaseDadosView
            ...
            criticClients={clients}
            onAddAuditNote={(clientId, note) =>
              handleAddAuditNote(clientId, note, adminProfile.name)
            }
            searchValue={searchValue}
            vehicleRegistries={vehicleRegistries}
            onAddVehicleRegistry={handleAddVehicleRegistry}
  ...
  return (
    ...
    >
      ...
      <main
        className={
          currentView === "roteirizacao"
            ? "p-1 w-full max-w-none"
            : "p-4 sm:p-6 md:p-8 max-w-7xl mx-auto min-h-[calc(100vh-4rem)]"
        }
      >
        {renderActiveView()}
      </main>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-surface-container border border-outline-variant max-w-sm w-full rounded-2xl p-6 md:p-8 text-center shadow-2xl relative overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <p className="text-xs text-[var(--router-text-soft)] mb-5 leading-normal">
              Insira a senha de usuário master para habilitar os recursos
              administrativos.
            </p>
